...
| config | object | `{"ADAPTER_OVERRIDE_ENABLED":"false","APP_PUBLIC_URL":"","AUDIT_AUTO_INTERCEPTOR_ENABLED":"false","AUDIT_PARTITION_CRON":"0 3 * * *","AUDIT_PARTITION_MONTHS_AHEAD":"3","CONNECTOR_ENCRYPTION_KEYS_PATH":"/etc/connector/encryption-keys.json","DB_HOST":"","DB_LOGGING":"false","DB_NAME":"dc_common_service","DB_PORT":"5432","DB_SYNCHRONIZE":"false","JWT_AUDIENCE":"https://digital-trust-common-service","LOG_LEVEL":"info","NODE_ENV":"production","OIDC_GRANT_TYPES":"client_credentials,authorization_code,refresh_token","OIDC_KEYS_PATH":"/etc/oidc/oidc-keys.json","PORT":"3000","RATE_LIMIT_ENABLED":"true","RATE_LIMIT_GLOBAL_PER_MINUTE":"2000","RATE_LIMIT_HIT_RETENTION_MINUTES":"5","RATE_LIMIT_PREMIUM_PER_MINUTE":"1000","RATE_LIMIT_PRUNE_CRON":"0 * * * *","RATE_LIMIT_STANDARD_PER_MINUTE":"100","RATE_LIMIT_WINDOW_MS":"60000","SWAGGER_ENABLED":"true","SWAGGER_JSON_ENABLED":"true"}` | Non-secret application configuration, rendered into a ConfigMap and injected as environment variables into all containers. |
| config.APP_PUBLIC_URL | string | `""` | This app's own externally-reachable origin, no path (e.g. https://api.example.com). Required in production: OidcConfigService throws at startup if unset. The OIDC provider is mounted at `${APP_PUBLIC_URL}/oidc`. |
...
| otel.logsExporter | string | `"none"` | OTEL_LOGS_EXPORTER for both the API and Worker. `none` on purpose, and it must be set explicitly: deployed, logs go to stdout and Alloy ships them to Loki, but leaving this unset does not mean "off" — the SDK's own default is `otlp`, so it builds a logger provider and the pino instrumentation posts every log line to the collector's `/v1/logs` as well. That is the same lines twice, over the wire and through Alloy, for no added signal. Trace correlation is unaffected: `trace_id` and `span_id` come from the instrumentation's pino mixin, not from log sending. |
| otel.metricsExporter | string | `"otlp"` | OTEL_METRICS_EXPORTER for both the API and Worker. |
