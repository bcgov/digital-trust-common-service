...
export interface OidcTenantUserPort {
   ...
   * Returns the rows it changed, oldest first; empty when there was nothing to
   * claim. At most one per tenant: invitations in soft-deleted tenants, and in
   * tenants where this identity already has a row, are left alone. The rows
   * carry no `tenant` relation.
   */
