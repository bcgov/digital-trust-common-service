...
- **Compose stack changes** additionally run `compose-smoke.yml`, which copies `.env.example` to `.env`, generates OIDC signing keys, brings the stack up, and asserts that the app, Keycloak and the Caddy front door all answer and that `migrate` and `seed` exited 0. Nothing else in CI reads `docker-compose.yml` or `.env`. It is path-filtered to `docker-compose.yml`, `.env.example`, `Dockerfile`, `caddy/**`, `keycloak/**`, `config/**` and the key-generation script, and is deliberately **not** a required check: a path-filtered workflow that does not run reports nothing, which would block every PR that leaves those files alone.
