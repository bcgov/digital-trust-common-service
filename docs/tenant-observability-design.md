...
**Issue**: [bcgov/digital-trust-common-service#103](https://github.com/bcgov/digital-trust-common-service/issues/103)
...
This document captures the research and the resulting design decisions for enabling **tenant-scoped observability** — letting a tenant self-serve access to dashboards scoped to only their own data. For the MVP the relevant services are **digital-trust-common-service** (greenfield NestJS service) and **Traction (ACA-Py)**, its sole credential-agent backend.
...
> **Target change**: the tenant-facing design replaces Grafana's IdP — instead of authenticating directly against Keycloak, Grafana becomes an OIDC client of **digital-trust-common-service's `oidc-provider`** (which federates upstream to Keycloak). See the Auth Integration section below.
...
**Cross-tenant querying**: tenant IDs can be pipe-joined in the header (`X-Scope-OrgID: a|b|…`) when `multi_tenant_queries_enabled: true` is set. **This is required even for MVP**: each tenant's scope already spans **two** Loki tenants — its digital-trust-common-service stream (under `tenant_id`) and its Traction stream (under `traction_tenant_id`) — so the gateway sets `X-Scope-OrgID: tenant_id|traction_tenant_id`. Platform dashboards pipe-join across all tenants the same way.
...
2. **One Loki datasource** on that instance, with **"Forward OAuth Identity"** (`oauthPassThru: true`) enabled — Grafana forwards the logged-in user's **app-issued access token** (from digital-trust-common-service's `oidc-provider`) with every datasource request.
3. **An identity-aware gateway** (small NestJS/Go service, or nginx `auth_request` + a resolver) sits between that datasource and Loki. Per request it: validates the app-issued JWT against digital-trust-common-service's JWKS (same validation as the API's guard, AU-03); **requires the `logs:read` scope and fails closed if absent** (authorize, don't just authenticate); reads `tenant_id` from the token claim; looks up `traction_tenant_id` in `ConnectorCredential` (cached); sets `X-Scope-OrgID: tenant_id|traction_tenant_id` (both the digital-trust-common-service and Traction streams); strips the Authorization header; forwards to Loki. It never reads the query.
...
**Per-tenant footprint in this model: zero Grafana objects.** Onboarding touches only the existing `ConnectorCredential` row (the tenant binding is the app-issued `tenant_id` claim digital-trust-common-service already mints).

Grafana is registered once as an `authorization_code` (interactive, PKCE) client of digital-trust-common-service's `oidc-provider`. A single static `org_mapping` rule routes all tenant users → the tenant org as Viewer, keyed on an app-issued claim — no Keycloak group design, and no per-tenant mapping.
...
| OIDC client of digital-trust-common-service | ✅ (generic OAuth) | Grafana points at the app `oidc-provider`, not Keycloak directly |
...
digital-trust-common-service is the source of truth for tenant lifecycle. When a tenant is provisioned/configured, downstream actions fire — as part of the same onboarding transaction digital-trust-common-service already runs (it is greenfield; design the hooks in).
...
- [ ] Gateway `ConnectorCredential` lookup: direct DB read vs. an internal API on digital-trust-common-service; cache TTL and invalidation on connector updates.
...
Tenant end-users authenticate through **digital-trust-common-service's `oidc-provider`**, which federates upstream to Keycloak for the actual login. Grafana is an OIDC client of digital-trust-common-service — not of Keycloak directly. On login:
...
### digital-trust-common-service oidc-provider integration specifics
...
### Tracing for MVP: digital-trust-common-service ↔ Traction
...
- **Our side** (digital-trust-common-service, and its outbound HTTP to the Traction API): covered by OTel auto-instrumentation (OB-01), plus custom spans for credential operations (OB-02). `tenant.id` is set as a span attribute at the request boundary.
...
───────────────────────────────────────────────────
 digital-trust-common-service ──HTTP (sub-wallet bearer token)──▶ Traction / ACA-Py
...
Phase 2 (near-term) → DEPENDS ON the auth foundation landing (oidc-provider, JWT access tokens,
                      ...
                      (OIDC client of digital-trust-common-service) + datasource + generic dashboards;
                      Loki NetworkPolicy hardening (dev/test).

Phase 3 (parallel)  → OTel auto-instrumentation for digital-trust-common-service (our side only; ACA-Py is
...
| Grafana as OIDC client of digital-trust-common-service | Medium | Low | Register interactive client; JWT access tokens with `tenant_id` + `aud` |
