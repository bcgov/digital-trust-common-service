...
5. Enforce the repository policy: no issue references, issue names, or issue numbers may appear in code comments or other committed repository content. This includes this repository's work-item codes (a short uppercase letter prefix, a hyphen, and digits, e.g. `AU-04`, `AG-01`, `IN-11`), generic issue/ticket references (`#1234`, `GH-1234`, `JIRA-123`), and any other tracker identifier — scan comments and docstrings specifically for this pattern, not only for the word "issue". Newly introduced tracking references in source, comments, documentation, tests, fixtures, configuration, generated output, or user-facing messages are blocker findings; recommend removing them. `Closes #1234` and a work-item code such as `AU-04` are allowed in the PR description and commit footer because that is PR/commit metadata, not repository content.
