...
import { OperationResult, OperationState } from '../operation/operation.entity';
...
export class ProtocolStateChangeService {
  ...
  private async applyOperationOutcome(
    ...
    const updated = await this.operationService.transitionStateIfForward(
      ...
      this.resolveOperationResult(data, outcome),
      manager,
    );
   ...
   * followed.
   *
   * This is a second, distinct state write from the action Operation's own
   * one in applyOperationOutcome, but process()'s single webhook-dispatch
   * call only ever reports that first transition (keyed to data.externalId,
   * which the offer and action Operations share). Left alone, this offer
   * transition would break the "every actual transition is dispatched"
   * contract that call documents. Dispatched here with its own event/
   * resourceId instead — not outcome.event/data.externalId, which name the
   * action Operation's own transition — so a consumer never sees the same
   * event/resource pair reported twice for two different Operations. No
   * in-process EventEmitter2.emit: not one of the six named domain events,
   * same as settleBatchParent's operation.batch.* dispatch below.
   */
  private async settleRelatedOfferOperation(
    ...
    await this.jobsService.sendInTransaction(
      ...
      {
        tenantId: data.tenantId,
        event: `operation.offer.${outcome.operationState}`,
        resourceId: offerOperation.id,
        occurredAt: new Date().toISOString(),
      },
    );
  }
   ...
   * pending/processing (CredentialActionService follows the same contract),
   * so a PROCESSING outcome must not leak the raw webhook payload as if it
   * were a terminal result.
   */
  private resolveOperationResult(
    data: ProtocolStateChangeJobData,
    outcome: ProtocolOutcome,
  ): OperationResult {
    if (outcome.operationState === OperationState.FAILED) {
      return {
    ...
    if (outcome.operationState === OperationState.PROCESSING) {
      return null;
    }
